...
assure_path_exists("/home/admin238/Downloads/Face-Recognition-Lock-Unlock-Screen/trainer/")

recognizer.read('/home/admin238/Downloads/Face-Recognition-Lock-Unlock-Screen/trainer/trainer.yml')  #load training model

cascadePath = "/home/admin238/Downloads/Face-Recognition-Lock-Unlock-Screen/haarcascade_frontalface_default.xml"  #cascade path
...
while True:
    ...
    for(x,y,w,h) in faces:
        ...
        if(confidence<50):                 #confidence usually comes greater than 80 for strangers
            Id = "Hello user + {0:.2f}%".format(round(100 - confidence, 2)) 
            print("Verified")
            print(confidence)
            counter_correct += 1
        ...
        else:                #confidence usually comes greater than 80 for strangers
            counter_wrong += 1
            print("Wrong")
            Id = "Unknown + {0:.2f}% + person".format(round(100 - confidence, 2)) 
            print(confidence)
            print("counter_wrong - " + str(counter_wrong))
            cv2.rectangle(im, (x-22,y-90), (x+w+22, y-22), (0,0,255), -1)
            cv2.putText(im, str(Id), (x,y-40), font, 1, (0,0,0), 2)                              #confidence usually comes less than 80 for correct user(s)
                
        if(counter_wrong == 3):
            pyautogui.moveTo(48,748)
            pyautogui.click(48,748)
            pyautogui.typewrite("Lock Screen")
            cam.release()
            cv2.destroyAllWindows()
            #ctypes.windll.user32.LockWorkStation()
            sys.exit()
